...
void initilize(){
    insert("KEYWORD","for",263);
    insert("KEYWORD","do",264);
    insert("KEYWORD","while",265);
    insert("KEYWORD","if",266);
    insert("KEYWORD","else",267);
    insert("KEYWORD","return",268);
    insert("KEYWORD","void",269);
    printf("\n\n");
    printf("------------------Input--------------------\n");

}

void insert(char * type,char * value,int tok){
    char key[100];
    strcpy(key,type);
    strcat(key,value);
    int hval=hash(key);
...
void reAllocate(){
    ...
    for(i=0; i<oldS; i++){
        if(strcmp(temp_table[i].value,"")!=0)
            insert(temp_table[i].type,temp_table[i].value,temp_table[i].token);
    }
    free(temp_table);
}
